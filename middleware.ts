...
export default async function middleware(request: NextRequest) {
  ...
  if (host.endsWith(`.${MAIN_DOMAIN}`)) {
    ...
    if (subdomain && !RESERVED_SUBDOMAINS.includes(subdomain)) {
      ...
      // Pass x-partner-pathname on the REQUEST (not response) so it is
      // visible via headers() in server component layouts. Response headers
      // from NextResponse.rewrite() are not forwarded to the destination.
