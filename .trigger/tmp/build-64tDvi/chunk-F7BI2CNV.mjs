...
// lib/push.ts
init_esm();
var import_web_push = __toESM(require_src2());
...
function initVapid() {
  ...
  import_web_push.default.setVapidDetails("mailto:" + mail, pub, priv);
  vapidInitialised = true;
  return true;
}
__name(initVapid, "initVapid");
...
async function sendPushToUser(supabase, userId, payload) {
  if (!initVapid()) return;
  const { data: subs, error } = await supabase.from("push_subscriptions").select("id, subscription").eq("user_id", userId);
  if (error) {
  ...
  await Promise.all(
    subs.map(async (row) => {
      const ok = await sendPush(row.subscription, payload);
...
*/
//# sourceMappingURL=chunk-F7BI2CNV.mjs.map
