...
// src/trigger/analytics-reporter.ts
init_esm();
...
var analyticsReporterAgent = schedules_exports.task({
  ...
  run: /* @__PURE__ */ __name(async () => {
    ...
    const signupChange = lastWeekSignups > 0 ? Math.round((thisWeekSignups - lastWeekSignups) / lastWeekSignups * 100) : 0;
    const thisWeekSessions = sessionsThisWeek.count || 0;
    const lastWeekSessions = sessionsLastWeek.count || 0;
    const sessionChange = lastWeekSessions > 0 ? Math.round((thisWeekSessions - lastWeekSessions) / lastWeekSessions * 100) : 0;
    const { data: wacuData } = await supabase.from("coaching_sessions").select("user_id").gte("created_at", weekAgo);
    const wacu = new Set((wacuData || []).map((r) => r.user_id)).size;
    const { data: hotLeads } = await supabase.from("profiles").select("full_name, lead_score, subscription_status").gte("lead_score", 70).in("subscription_status", ["inactive", "cancelled"]).order("lead_score", { ascending: false }).limit(5);
    const metrics = {
...
Return plain text only (no JSON, no markdown headers).`
    ...
    const narrative = claudeRes.content[0].type === "text" ? claudeRes.content[0].text : "Weekly metrics collected. Review your dashboard for full details.";
    const changeArrow = /* @__PURE__ */ __name((n) => n > 0 ? `↑${n}%` : n < 0 ? `↓${Math.abs(n)}%` : "—", "changeArrow");
    const html = `
<!DOCTYPE html>
<html>
...
//# sourceMappingURL=analytics-reporter.mjs.map
