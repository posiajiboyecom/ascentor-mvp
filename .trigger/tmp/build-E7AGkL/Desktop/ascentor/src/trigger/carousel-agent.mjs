...
var BUCKET = "content-media";
var SLIDE_FOLDER = "carousel-slides";
var COST_PER_IMAGE = 0.063;
function extractJSON(raw) {
  ...
  if (s !== Infinity) {
    ...
    if (e > s) {
      ...
      try {
        return JSON.parse(repair(text.slice(s, e + 1)));
      } catch {
      }
    }
  }
...
var SYSTEM = "You are a JSON API. Respond with ONLY valid raw JSON. No markdown fences, no text before or after JSON. Represent all newlines inside strings as \\n. Never truncate.";
var FORMULA = "MASTER FORMULA — every hook maps to:\n[PERSON] + [CONFLICT] → [ASCENTOR MOMENT] → [MIND CHANGED]\n\nPERSON: Boss, manager, co-founder, investor, mentor, team, skip-level\nCONFLICT: Said no · didn't believe · passed them over · was about to quit\nASCENTOR MOMENT: Mentor asked · I showed them · the framework revealed\nMIND CHANGED: They agreed · I got promoted · everything shifted\n\nHOOK RULES: max 18 words · named person · conflict present · no features/pricing · no emojis · no exclamation marks · global framing";
async function getHooks(pillar, platform, postCount, provided) {
...
["hook 1","hook 2",...]`
  ...
  const all = extractJSON(raw);
  const selected = [];
  for (let i = 0; i < postCount; i++) {
    const opts = all.slice(i * 5, i * 5 + 5);
    selected.push(
      opts.find((h) => h.split(" ").length <= 18) ?? opts[0] ?? `My mentor changed my approach to ${pillar}.`
    );
  }
...
async function buildBrief(hook, pillar, platform) {
  const cta = {
  ...
  const res = await anthropic.messages.create({
    ...
    messages: [{
      role: "user",
      content: `You are a world-class creative director and visual storyteller for a career platform serving ambitious African professionals globally.
...
Across 6 slides you must use: different locations, different times of day, different shot types (wide/close/overhead/low angle), different colour temperatures (warm/cool/neutral), different number of people (solo/two/group). No two slides should look like they could be from the same photoshoot.

TEXT RULES:
- Slide texts: punchy, max 15 words each. First person. No corporate language.
- Slide 6 text = exactly: "${cta[platform]}"

caption: First person, past tense, 100–150 words. Conversational. Raw. No emojis, no exclamation marks. No words: leverage/synergy/impactful/journey/pivot/game-changer. Use \\n for line breaks.
...
async function generateImages(brief, jobId) {
  const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
  const dir = path2.join("/tmp", "ascentor-carousel", jobId);
  fs.mkdirSync(dir, { recursive: true });
  const localPaths = [];
  let cost = 0;
  for (let i = 0; i < brief.slides.length; i++) {
    const slide = brief.slides[i];
    const prompt = slide.imagePrompt ? `${slide.imagePrompt}

No text, words, letters, or captions anywhere in the image. Photorealistic. Cinematic quality. Portrait orientation.` : `Professional career moment. Cinematic. Black African professional. No text. Portrait orientation.`;
    ...
    try {
      const response = await openai.images.generate({
        ...
        prompt,
        size: "1024x1024",
        // square — cheapest size, upgrade to 1024x1536 when ready
        quality: "medium",
        // $0.011/image — upgrade to "medium" ($0.063) or "high" ($0.25) when ready
        n: 1
      });
      ...
      const rawPath = path2.join(dir, `slide-${i + 1}-raw.jpg`);
      const finalPath = path2.join(dir, `slide-${i + 1}.jpg`);
      fs.writeFileSync(rawPath, imageBuffer);
      await applyOverlay(rawPath, slide.text, finalPath);
      localPaths.push(finalPath);
      cost += COST_PER_IMAGE;
      if (i < brief.slides.length - 1) await sleep2(2e3);
    } catch (err) {
...
async function applyOverlay(inputPath, text, outputPath) {
  try {
    ...
    const escape2 = /* @__PURE__ */ __name((s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;"), "escape");
    const svgLines = lines.map((line, i) => {
      const y = TEXT_Y + i * LINE_HEIGHT;
      const l = escape2(line);
      return [
    ...
    await sharp(inputPath).composite([{ input: Buffer.from(svg), blend: "over" }]).jpeg({ quality: 92 }).toFile(outputPath);
...
__name(applyOverlay, "applyOverlay");
async function uploadImages(localPaths, jobId) {
  ...
  for (let i = 0; i < localPaths.length; i++) {
    ...
    const { error } = await supabase.storage.from(BUCKET).upload(storagePath, fs.readFileSync(p), {
...
async function uploadTextFile(brief, jobId) {
  try {
    const lines = [
      `CAROUSEL — ${brief.hook}`,
      ...
      "",
      ...brief.slides.map(
        (s) => `SLIDE ${s.slide} [${s.purpose.toUpperCase()}]
${s.text}
`
      ),
      `${"─".repeat(60)}`,
    ...
    const { error } = await supabase.storage.from(BUCKET).upload(storagePath, Buffer.from(lines, "utf-8"), {
      contentType: "text/plain",
      upsert: false
    });
    if (error) {
      logger.warn(`[Carousel] txt upload failed: ${error.message}`);
      return null;
    }
    const { data: { publicUrl } } = supabase.storage.from(BUCKET).getPublicUrl(storagePath);
...
__name(uploadTextFile, "uploadTextFile");
async function saveToCalendar(brief, urls, textFileUrl, week, triggeredBy) {
...
__name(saveToCalendar, "saveToCalendar");
var carouselAgentTask = task({
  id: "carousel-agent",
  maxDuration: 300,
  run: /* @__PURE__ */ __name(async (payload) => {
    const {
      ...
      platform,
      postCount = 3,
      week = Math.ceil((/* @__PURE__ */ new Date()).getDate() / 7),
      hooks,
      triggeredBy = "manual"
    } = payload;
    ...
    return {
      ...
      nextStep: "Open Content Calendar → filter Instagram Carousel → review → approve → Queue Social → Buffer schedules"
    };
  }, "run")
});
function sleep2(ms) {
...
//# sourceMappingURL=carousel-agent.mjs.map
