...
// src/trigger/email-templates.ts
init_esm();
var B = {
  black: "#0C0B08",
  gold: "#E8A020",
...
__name(preheader, "preheader");
function newsletterLayout(opts) {
  const {
    preheaderText,
    eyebrow = "THE RISE LETTER · ASCENTOR · FOR THE AMBITIOUS",
    headline,
    subhead,
    meta = "VOL 1 · 5-MIN READ",
    pullQuote,
    body,
    ctaText = "Read on Ascentor →",
    ctaUrl = "https://ascentorbi.com/blog",
    unsubscribeToken = ""
  } = opts;
      ...
      <table width="100%" cellpadding="0" cellspacing="0" role="presentation" class="card"
        ...
        <tr><td class="footer-pad" style="padding:28px 40px;border-top:1px solid ${B.border};text-align:center">
          ...
          <p style="margin:12px 0 0;font-family:'DM Mono',monospace,sans-serif;font-size:9px;color:${B.faint}">
            © ${(/* @__PURE__ */ new Date()).getFullYear()} Ascentor · ascentorbi.com
          </p>
        </td></tr>
...
__name(newsletterLayout, "newsletterLayout");
      ...
      <table width="100%" cellpadding="0" cellspacing="0" role="presentation" class="card"
        ...
        <tr><td style="padding:20px 40px 28px;border-top:1px solid ${B.border};text-align:center">
          <p style="margin:0;font-family:'DM Mono',monospace,sans-serif;font-size:10px;color:${B.faint}">
            ...
            <a href="https://ascentorbi.com/privacy" style="color:${B.faint};text-decoration:none">Privacy</a>
            &nbsp;·&nbsp;
            © ${(/* @__PURE__ */ new Date()).getFullYear()} Ascentor
          </p>
        </td></tr>
...
__name(transactionalLayout, "transactionalLayout");
function parseNewsletterBody(raw) {
...
__name(parseNewsletterBody, "parseNewsletterBody");
function formatInline(text) {
  return text.replace(/\*\*(.+?)\*\*/g, `<strong style="color:${B.cream};font-weight:700">$1</strong>`).replace(/\*(.+?)\*/g, `<em style="font-style:italic;color:${B.cream}">$1</em>`).replace(/`(.+?)`/g, `<code style="font-family:'DM Mono',monospace;font-size:13px;color:${B.gold};background:rgba(232,160,32,0.08);padding:1px 5px;border-radius:3px">$1</code>`);
}
__name(formatInline, "formatInline");
function newsletterEmail(subject, content, firstName, opts) {
...
__name(newsletterEmail, "newsletterEmail");
...
function coachingSummaryEmail(name, summary, keyTakeaways, nextSteps) {
  const firstName = name?.split(" ")[0] || "there";
...
__name(coachingSummaryEmail, "coachingSummaryEmail");
function goalReminderEmail(name, goals, daysSinceActive) {
  const firstName = name?.split(" ")[0] || "there";
  ...
  const urgency = daysSinceActive > 14 ? `It has been ${daysSinceActive} days. That is long enough.` : `It has been ${daysSinceActive} days since your last session.`;
  return {
...
__name(goalReminderEmail, "goalReminderEmail");
export {
