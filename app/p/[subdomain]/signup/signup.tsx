...
// ============================================================
// PARTNER SIGNUP — app/p/[subdomain]/signup/page.tsx
//
// Replaces the old re-export of the main Ascentor signup.
//
// WHITELIST ENFORCEMENT:
//   1. Before rendering: checks if the user is already logged in
//      (if so, redirects to partner dashboard)
//   2. On email submit: calls /api/auth/check-email to see if the
//      email is in partner_members for this partner. If not invited,
//      shows a "not on the list" message with a request-access CTA.
//   3. On successful signup: passes partner context through to
//      /auth/callback via the emailRedirectTo URL so the callback
//      can finalize membership.
//
// Visual style: inherits partner brand CSS vars from layout.tsx,
// matches the Ascentor auth page structure (two-panel layout).
// ============================================================
...
function PartnerSignupForm() {
  ...
  // Invite token from URL (/p/[subdomain]/join?partner=xxx populates this)
  ...
  async function checkWhitelist(emailToCheck: string): Promise<{ allowed: boolean; reason?: string }> {
    try {
      ...
      // If check fails, allow through — don't block users due to our own errors
      return { allowed: true };
    }
  }
  ...
  async function handleSignUp(e: React.FormEvent) {
    ...
    // 3. Success — redirect to dashboard (Supabase email confirmation optional)
    router.push(redirectTo);
    setLoading(false);
  }

  // ── Handle OAuth signup ───────────────────────────────────
  // OAuth can't pre-check whitelist (we don't know the email before
  // the OAuth flow). The check happens in auth/callback instead.
