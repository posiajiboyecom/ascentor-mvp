...
export default function AIPersonaPage() {
  ...
  useEffect(() => {
    // FIX: ai_persona_prompt moved from brand JSONB → ai_config column.
    // Read from /api/partner/config which returns the full partner row,
    // then pull ai_config.ai_persona_prompt. Falls back gracefully for
    // partners whose prompt still lives in brand (pre-migration data).
    fetch('/api/partner/config')
      .then((r) => r.json())
      .then((data) => {
        const aiConfig = data.ai_config || {};
        const brand    = data.brand    || {};
        // ai_config is canonical; brand.ai_persona_prompt is legacy fallback
        setPrompt(aiConfig.ai_persona_prompt || brand.ai_persona_prompt || DEFAULT_PERSONA);
        setLoading(false);
      })
      .catch(() => {
