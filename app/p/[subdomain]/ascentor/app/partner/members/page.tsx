...
export default async function PartnerMembersPage() {
  ...
  // Member list now comes from partner_members joined to profiles,
  // not from profiles.tenant_id (old schema field that no longer exists).
  ...
  const { data: members, count } = partner
    ? await supabase
        .from('partner_members')
        .select(`
          id, email, role, status, joined_at,
          profiles ( full_name, subscription_plan, subscription_status, created_at )
        `, { count: 'exact' })
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            ...
            <tbody>
              {members.map((m: any) => {
                ...
                const c = statusColor(profile.subscription_status);
                return (
                  <tr key={m.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    ...
                    <td style={{ padding: '12px 16px', color: 'rgba(255,255,255,0.6)' }}>{profile.subscription_plan || 'free'}</td>
