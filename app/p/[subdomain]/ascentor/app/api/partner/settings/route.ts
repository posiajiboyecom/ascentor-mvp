...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (ai_persona_prompt !== undefined) {
      // FIX: ai_persona_prompt belongs in ai_config, NOT brand JSONB.
      // brand is purely UI/styling. ai_config owns all AI behaviour settings.
      // Merge so other ai_config fields added in future are preserved.
      const { data: current } = await supabase
        .from('partners')
        .select('ai_config')
        .eq('id', partner.id)
