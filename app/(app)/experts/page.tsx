...
export default function ExpertsPage() {
  ...
  async function loadData() {
    ...
    const [sessionsRes, regsRes, pastRes] = await Promise.all([
      ...
      supabase.from('expert_sessions').select('*').eq('status', 'completed').order('scheduled_at', { ascending: false }),
    ]);

    setExperts(sessionsRes.data || []);
    setRegistered(new Set(regsRes.data?.map((r: any) => r.session_id) || []));
    setRecordings(pastRes.data || []);
    setLoading(false);
  }
  ...
  return (
    <div className="animate-fade-up py-6">
      ...
      <div className="flex gap-1 mb-5 p-1 rounded-lg" style={{ background: 'var(--bg-input)' }}>
        <button onClick={() => setTab('upcoming')} className="flex-1 py-2 rounded-md text-xs font-semibold transition-all" style={{ background: tab === 'upcoming' ? 'var(--bg-card)' : 'transparent', color: tab === 'upcoming' ? 'var(--accent)' : 'var(--text-dim)' }}>Upcoming ({experts.length})</button>
        <button onClick={() => setTab('recordings')} className="flex-1 py-2 rounded-md text-xs font-semibold transition-all" style={{ background: tab === 'recordings' ? 'var(--bg-card)' : 'transparent', color: tab === 'recordings' ? 'var(--accent)' : 'var(--text-dim)' }}>Recordings ({recordings.length})</button>
      </div>

      {activeWebinar && (
        <div className="rounded-xl overflow-hidden mb-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--accent)' }}>
          <div className="flex justify-between items-center px-4 py-2" style={{ borderBottom: '1px solid var(--border)' }}>
            <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /><span className="text-xs font-semibold" style={{ color: 'var(--text)' }}>Live Session</span></div>
            <button onClick={() => setActiveWebinar(null)} className="text-xs px-2 py-1 rounded" style={{ color: 'var(--text-dim)' }}>✕ Close</button>
          </div>
          <div className="relative" style={{ paddingBottom: '56.25%' }}><iframe src={activeWebinar} className="absolute inset-0 w-full h-full" allow="microphone; camera; fullscreen" style={{ border: 'none' }} /></div>
        </div>
      )}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-2">
                      {live && isReg && expert.zoom_join_url ? (
                        <button onClick={() => window.open(expert.zoom_join_url, '_blank')} className="px-5 py-2 rounded-lg text-xs font-semibold flex items-center gap-1.5" style={{ background: 'var(--error)', color: '#fff' }}>🎥 Join Live Session</button>
                      ) : (
      ...
      {tab === 'recordings' && (
        <div className="flex flex-col gap-3">
          {recordings.length === 0 ? (
            <div className="text-center py-12"><div className="text-3xl mb-3">📹</div><p className="text-sm" style={{ color: 'var(--text-muted)' }}>No recordings yet. Check back after upcoming sessions.</p></div>
          ) : recordings.map((rec) => (
            <div key={rec.id} className="rounded-xl overflow-hidden" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
              {rec.recording_url && (
                <div className="relative" style={{ paddingBottom: '56.25%' }}><iframe src={extractYoutubeEmbed(rec.recording_url)} className="absolute inset-0 w-full h-full" allow="fullscreen" style={{ border: 'none' }} /></div>
              )}
              <div className="p-4">
                <h4 className="text-sm font-semibold" style={{ color: 'var(--text)' }}>{rec.title}</h4>
                <p className="text-xs mt-0.5" style={{ color: 'var(--text-dim)' }}>by {rec.expert_name} · {new Date(rec.scheduled_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
              </div>
            </div>
          ))}
        </div>
