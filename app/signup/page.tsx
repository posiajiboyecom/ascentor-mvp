...
function SignUpForm() {
  ...
  const planParam    = searchParams.get('plan');    // 'free' | 'builder' | 'pro' | 'elite'
  ...
  async function handleEmailSignUp(e: React.FormEvent) {
    ...
    if (error) {
      ...
      // FIX: Route based on chosen plan
      // Free plan → onboarding (which then goes to dashboard)
      // Paid plan → checkout with plan + billing pre-selected
      if (planParam === 'free' || !planParam) {
        router.push('/onboarding');
      } else {
        const billingQuery = billingParam ? `&billing=${billingParam}` : '';
        router.push(`/checkout?plan=${planParam}${billingQuery}`);
      }
    }
    setLoading(false);
  }
        ...
        <div className="asc-form-panel">
          <div className="asc-form-inner">
            ...
            <div className="asc-oauth">
              {/* FIX: compute redirect so OAuth carries plan+billing through the callback */}
              {(() => {
                const oauthRedirect = (planParam && planParam !== 'free')
                  ? `${typeof window !== 'undefined' ? window.location.origin : ''}/auth/callback?next=${encodeURIComponent(`/checkout?plan=${planParam}${billingParam ? `&billing=${billingParam}` : ''}`)}`
                  : undefined; // undefined → sso.tsx uses default /auth/callback
                return (
                  <>
                    <OAuthButton provider="google"        onError={setError} redirectTo={oauthRedirect} />
                    <OAuthButton provider="linkedin_oidc" onError={setError} redirectTo={oauthRedirect} />
                  </>
                );
              })()}
            </div>
