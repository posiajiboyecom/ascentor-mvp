...
// ─────────────────────────────────────────────────────────────
// Community Page — live data from Supabase `cohorts` table.
// Replaces the hardcoded MEMBERS / INITIAL_POSTS mock.
// Users can join cohorts (subject to free-tier limit of 3),
// then navigate into the real-time feed at /community/[cohortId].
// ─────────────────────────────────────────────────────────────
...
export default function CommunityPage() {
  ...
  async function loadData() {
    ...
    setSubscription({ plan: profile?.subscription_plan || 'free', hasAccess: !!hasAccess });
    setLoading(false);
  }
  ...
  async function joinCohort(cohortId: string) {
    ...
    try {
      ...
      const { data: existing } = await supabase
        ...
        .eq('cohort_id', cohortId).eq('user_id', user.id).maybeSingle();
      ...
      const { error } = await supabase.from('cohort_members').insert({ cohort_id: cohortId, user_id: user.id });
      if (!error) {
        setMyCohortIds(prev => new Set([...prev, cohortId]));
        setCohorts(prev => prev.map(c => c.id === cohortId ? { ...c, member_count: (c.member_count || 0) + 1 } : c));
  ...
  async function leaveCohort(cohortId: string, e: React.MouseEvent) {
    ...
    const { error } = await supabase.from('cohort_members').delete()
      .eq('cohort_id', cohortId).eq('user_id', user.id);

    if (!error) {
      setMyCohortIds(prev => { const next = new Set(prev); next.delete(cohortId); return next; });
        ...
        <div className="relative mb-3">
          ...
          {search && (
            <button onClick={() => setSearch('')}
              style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-dim)', fontSize: 16, lineHeight: 1, background: 'none', border: 'none', cursor: 'pointer' }}>
              ×
            </button>
          )}
        </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-3 text-xs" style={{ color: 'var(--text-dim)' }}>
                      <span>
                        <svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ display: 'inline', marginRight: 3, verticalAlign: 'middle' }}>
