  ...
  function subscribeRealtime() {
    ...
    const repliesChannel = supabase
      ...
      .on('postgres_changes', {
        ...
        // Skip own replies — already handled optimistically in createReply()
        ...
        setPosts(prev => prev.map(p => {
          ...
          // Always increment reply_count regardless of whether replies panel is open
      ...
      .on('postgres_changes', {
        ...
        // Catches reply_count updates pushed from DB trigger
        const updated = payload.new as any;
        setPosts(prev => prev.map(p =>
          p.id === updated.id
            ? { ...p, reply_count: updated.reply_count ?? p.reply_count, upvotes: updated.upvotes ?? p.upvotes }
            : p
        ));
      })
      .on('postgres_changes', {
