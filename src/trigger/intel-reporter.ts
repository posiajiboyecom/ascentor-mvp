// ═══════════════════════════════════════════════════════════
// Agent 8: Intel Reporter
// Runs daily at 7am UTC (Mon–Fri) + 8am weekends.
// Pulls full platform data, generates AI BI report via Claude,
// saves to platform_snapshots table.
// Intel page reads from this cache — zero live Claude calls.
//
// Cost: ~$0.023/run × 7 days = ~$0.16/week = ~$0.70/month
// ═══════════════════════════════════════════════════════════
...
// ── Same data-fetching logic as the Intel page ──────────────
async function fetchPlatformData() {
  ...
  const [
    ...
    auditRes, deletionRes, mentorRes,
  ...
  const scores = (leadRes.data || []).map((r: any) => Number(r.lead_score));
  const avgLeadScore = scores.length > 0 ? Math.round(scores.reduce((a: number, b: number) => a + b, 0) / scores.length) : 0;
  const cohortList = cohortsRes.data || [];
  const avgCohortSize = cohortList.length > 0
    ? Math.round(cohortList.reduce((s: number, c: any) => s + (c.member_count || 0), 0) / cohortList.length)
    : 0;
...
function buildPrompt(data: Awaited<ReturnType<typeof fetchPlatformData>>): string {
...
// ── Main scheduled task ─────────────────────────────────────
export const intelReporterAgent = schedules.task({
  ...
  run: async () => {
    ...
    const claudeRes = await anthropic.messages.create({
      model: "claude-haiku-4-5-20251001",  // Haiku is 15x cheaper, fully capable for this structured task
      max_tokens: 2000,
    ...
    const tokenCost = {
      ...
      cost_usd:      Math.round(costUsd * 100000) / 100000,  // 5 decimal places
    };

    const durationMs = Date.now() - startTime;

    // 4. Save to platform_snapshots
    ...
    // 5. Prune old snapshots — keep last 30 only
