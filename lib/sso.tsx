...
// ============================================================
// SSO Module — Google + LinkedIn + Discord OAuth
// ============================================================

const supabase = createClient();

type OAuthProvider = 'google' | 'linkedin_oidc' | 'discord';
...
export async function signInWithSSO(provider: OAuthProvider, redirectTo?: string): Promise<SSOResult> {
  try {
    ...
    if (error) {
      return { success: false, error: getReadableError(provider, error.message) };
    }
...
function getReadableError(provider: OAuthProvider, raw: string): string {
  if (raw.includes('provider is not enabled')) {
    const names: Record<OAuthProvider, string> = { google: 'Google', linkedin_oidc: 'LinkedIn', discord: 'Discord' };
    return `${names[provider]} sign-in is not configured. Please contact support.`;
  }
